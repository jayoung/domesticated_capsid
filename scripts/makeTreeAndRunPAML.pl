...
#### usage:  pp_makeTreeAndRunPAML.pl  alignmentFile(s)*.fa
#takes a bunch of alignments in fasta format. for each one, make a tree (if it does not already exist) and runs paml. see pp_makeTreeAndRunPAML_sbatchWrapper.pl if I want to use the cluster.
...
my $masterPipelineDir = "/fh/fast/malik_h/user/jayoung/paml_screen/githubVersion/janet_pamlPipeline";

my $codemlCTLtemplateFile = "$masterPipelineDir/pp_templates/master_sitewise_codeml.ctl";

#my @modelsToRun = ("0","0fixNeutral","1","2","7","8","8a");
#my @modelsToRun = ("0","1","2","7","8","8a");
my @modelsToRun = ("0");

### I used to encode sbatch use within this script, but now I do it using pp_makeTreeAndRunPAML_sbatchWrapper.pl
#my $use_sbatch = "no";
...
if (!-e $codemlCTLtemplateFile) {
    die "\n\nterminating pp_makeTreeAndRunPAML.pl - cannot open template file $codemlCTLtemplateFile\n\n";
}
undef $/;
...
foreach my $alignmentFile (@ARGV) {
    ...
    if (!-e $alnFilePhylipFormat) {
        system("$masterPipelineDir/pp_scripts/pp_fasta2pamlformat.bioperl $alnFileWithoutDir")
    }
    ## xxx here - I could open up the .phy file and make sure the alignment length is a multiple of three
    
    ## find or make the tree file using pp_runPHYML.pl
    ...
    if (!-e $treeFile1) { 
        system("$masterPipelineDir/pp_scripts/pp_runPHYML.pl $alnFilePhylipFormat")
    }
    if (!-e $treeFile2) {
        die "\n\nterminating pp_makeTreeAndRunPAML.pl - tree file $treeFile2 does not exist\n\n";
    }
